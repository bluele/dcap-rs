use crate::types::{enclave_identity::EnclaveIdentityV2, TcbStatus, quotes::body::EnclaveReport, ValidityIntersection};
use crate::utils::crypto::verify_p256_signature_bytes;
use crate::X509Certificate;

pub fn validate_enclave_identityv2(enclave_identityv2: &EnclaveIdentityV2, sgx_signing_pubkey: &X509Certificate, current_time: u64) -> Option<ValidityIntersection> {
    let issue_date_seconds = enclave_identityv2.enclave_identity.issue_date().unwrap().timestamp().try_into().unwrap();
    let next_update_seconds = enclave_identityv2.enclave_identity.next_update().unwrap().timestamp().try_into().unwrap();
    ...
    if verify_p256_signature_bytes(&enclave_identityv2_signature_data, &enclave_identityv2_signature_bytes, sgx_signing_pubkey.public_key().subject_public_key.as_ref()) {
        Some(
